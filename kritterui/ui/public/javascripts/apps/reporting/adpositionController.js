...
controller('adpositionReportController',function ($scope, $http, ngTableParams, $modal) {
	$scope.reportType= "";
	...
	var tbparams = new ngTableParams({
      ...
      getData: function($defer, params) {   
		...
		$('#preloader').removeClass("hidden"); 
      	var action_url = $('#adpositionReportConfigForm').attr('action'); 
      	if($scope.getCSV){
       		action_url = action_url + "/CSV";
       	}
   		var data = $('#adpositionReportConfigForm').serialize();
	...
	$scope.adposition_by_pub_url = "/metadata/options/adpositionbypubids/";
	...
	$scope.adposition_msmodel = Object.create(MultiSelectModel);
	$scope.adposition_msmodel.init($scope.adposition_by_pub_url, 'valueArray', true, true);
	$scope.publishers_msmodel.addDependent("adposition", $scope.adposition_msmodel);
	
	$scope.country_msmodel = Object.create(MultiSelectModel);
	$scope.country_msmodel.init($scope.country_url, 'valueArray', true, true);
	
	$scope.state_msmodel = Object.create(MultiSelectModel);
	$scope.state_msmodel.init( $scope.state_url, 'valueArray', true, true);
	$scope.country_msmodel.addDependent("state", $scope.state_msmodel);
	
	$scope.city_msmodel = Object.create(MultiSelectModel);
	$scope.city_msmodel.init( $scope.city_url, 'valueArray', true, true);
	$scope.state_msmodel.addDependent("city", $scope.city_msmodel);
