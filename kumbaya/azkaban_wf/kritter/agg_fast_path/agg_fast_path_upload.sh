...
echo "${runtime}"
echo "find  /var/data/kritter/file_journal/  -name "fast_path*" -newermt "${runtime}" -type f|xargs ls -1tr|head -1|sed 's#.*/##'"
findout=`find  /var/data/kritter/file_journal/  -name "fast_path*" -newermt "${runtime}" -type f|xargs ls -1tr|head -1|sed 's#.*/##'`
echo "${findout}"
if echo "${findout}" | grep 'fast_path_'; then
    echo "here"
    #arrIN=${findout/__// }
    newtime=`stat -c %y /var/data/kritter/file_journal/${findout}`
    touch /var/data/kritter/fastpathagglastrun
    echo ${newtime} > /var/data/kritter/fastpathagglastrun
    process_time="${findout/fast_path__/}"
    filepath=${2}/${process_time}/fast_path/part*
    IFS=- # delimit on _
