 ...
 */
public class CountryUiIdAndCountryTableMappingCache extends MappingCache {
    ...
    private static void buildCache() {
        try (Connection con = DBConnector.getConnection(dbtype, dbhost, dbport, dbname, dbuser, dbpwd)) {
            PreparedStatement prepareStatement = con.prepareStatement("SELECT uc.id AS ui_country_id,uc.country_name AS uc_country_name,c.`country_name` AS c_country_name,uc.entity_id_set AS entity_id_set,c.`country_code` AS country_code FROM country AS c,(SELECT country_name,id,REPLACE(REPLACE(entity_id_set,'[',''),']','') AS entity_id_set FROM ui_targeting_country WHERE id <> -1) AS uc WHERE c.id = uc.entity_id_set;");
            ResultSet resultSet = prepareStatement.executeQuery();
            while (resultSet.next()) {
                mappingCache.put(resultSet.getInt("ui_country_id"), resultSet.getString("country_code"));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static String getCountryCode(Integer countryUiId) {
        return mappingCache.get(countryUiId);
    }
