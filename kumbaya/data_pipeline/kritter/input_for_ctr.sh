...
if [ "$2" = "" ]; then
    SCRIPT_PATH=src/main/pig/input_for_ctr.pig
fi

if [ "$3" = "" ]; then
    OUTPUT=/home/rohan/testdata/kritter/out/input_for_ctrjob
fi
...
MAPRED=/tmp/input_for_ctr_`date +%s`
pig_script=input_for_ctr
