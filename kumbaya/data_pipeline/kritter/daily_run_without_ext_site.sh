...
if [ "$2" = "" ]; then
    SCRIPT_PATH=src/main/pig/rollup_first_level_without_ext_site.pig
fi
...
echo "$PIG_HOME/bin/pig -x local -f $SCRIPT_PATH  --param INPUT_FILES="$INPUT_FILES" --param EXT_INP_FILES="$EXT_INP_FILES" --param OUTPUT="$OUTPUT" --param rolluptype="$ROLLUPTYPE" --param PROCESS_TIME="$PROCESS_TIME" --param EXT_SITE_INPUT_FILES="$EXT_SITE_INPUT_FILES" --param LIMITED_INPUT_FILES="$LIMITED_INPUT_FILES""

$PIG_HOME/bin/pig -x local -f $SCRIPT_PATH  --param INPUT_FILES="$INPUT_FILES" --param EXT_INP_FILES="$EXT_INP_FILES" --param OUTPUT="$OUTPUT" --param rolluptype="$ROLLUPTYPE" --param PROCESS_TIME="$PROCESS_TIME" --param EXT_SITE_INPUT_FILES="$EXT_SITE_INPUT_FILES" --param LIMITED_INPUT_FILES="$LIMITED_INPUT_FILES"
