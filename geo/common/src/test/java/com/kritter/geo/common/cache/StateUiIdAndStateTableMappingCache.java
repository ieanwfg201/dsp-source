 ...
 */
public class StateUiIdAndStateTableMappingCache extends MappingCache {
    ...
    private static void buildCache() {
        try (Connection con = DBConnector.getConnection(dbtype, dbhost, dbport, dbname, dbuser, dbpwd)) {
            PreparedStatement prepareStatement = con.prepareStatement("SELECT uc.id AS ui_state_id,uc.state_name AS uc_state_name,c.`state_name` AS c_state_name,uc.entity_id_set AS entity_id_set,c.`state_code` AS state_code FROM state AS c,(SELECT state_name,id,REPLACE(REPLACE(entity_id_set,'[',''),']','') AS entity_id_set FROM ui_targeting_state WHERE id <> -1) AS uc WHERE c.id = uc.entity_id_set;");
            ResultSet resultSet = prepareStatement.executeQuery();
            while (resultSet.next()) {
                mappingCache.put(resultSet.getInt("entity_id_set"), resultSet.getString("state_code"));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static String getStateCode(Integer stateUiId) {
        return mappingCache.get(stateUiId);
    }
