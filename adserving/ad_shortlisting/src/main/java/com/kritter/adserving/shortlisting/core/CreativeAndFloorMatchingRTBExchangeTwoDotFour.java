...
import com.kritter.bidrequest.entity.common.openrtbversion2_4.*;
...
public class CreativeAndFloorMatchingRTBExchangeTwoDotFour implements CreativeAndFloorMatchingRTBExchange
{
    ...
    public CreativeAndFloorMatchingRTBExchangeTwoDotFour(
                                                          String loggerName,
                                                         ...
                                                         ) throws Exception
    {
        ...
        BidRequestImpressionDTO[] bidRequestImpressionDTOs = bidRequestParentNodeDTO.getImp();
        ...
        for(BidRequestImpressionDTO bidRequestImpressionDTO : bidRequestImpressionDTOs)
        {
            ...
            BidRequestPMPDTO bidRequestPMPDTO = bidRequestImpressionDTO.getPmp();
            setPrivateDealInfoInRequest(bidRequestPMPDTO,request,bidRequestImpressionDTO.getId());
            ...
            if(bidRequestImpressionDTO.getVideo()!= null){
            ...
            BidRequestImpressionBannerObjectDTO bidRequestImpressionBannerObjectDTO =
                    bidRequestImpressionDTO.getBanner();
            ...
            Integer width = bidRequestImpressionBannerObjectDTO.getW();
            Integer height = bidRequestImpressionBannerObjectDTO.getH();
            ...
            Integer[] blockedCreativeTypes = bidRequestImpressionBannerObjectDTO.getBtype();

            Set<Integer> blockedCreativeTypeSet = null;
            if(null != blockedCreativeTypes)
                blockedCreativeTypeSet = new HashSet<Integer>(Arrays.asList(blockedCreativeTypes));

            boolean isBannerAllowed = true;
            boolean isRichmediaAllowed = true;

            isBannerAllowed = ValidateBlockedCreativeType.validateInteger(request, blockedCreativeTypeSet, logger, BANNER_CREATIVE_TYPE);
            ...
            if(isRichmediaAllowed == true){
                isRichmediaAllowed = ValidateRichMediaType.validateBCATInteger(request, logger, blockedCreativeTypeSet, RICHMEDIA_CREATIVE_TYPE);
            }
            ...
            for(ResponseAdInfo responseAdInfo : response.getResponseAdInfo())
            {
                    ...
                    for(Integer bannerId : bannerUriIds)
                    {
                        ...
                        if(sizeFoundUsingRequestWidthHeightArray)
                        {
                            logger.debug("Creative Banner id:{} found for this impressionId:{} using sizeFoundUsingRequestWidthHeightArray" ,
                                    bannerId,bidRequestImpressionDTO.getId());
                ...
                else
                {
                    ...
                    ReqLog.debugWithDebugNew(logger, request, "Ad id : {} has ecpm {} while the floor is {}. The ad " +
                            ...
                            bidRequestImpressionDTO.getBidfloor());

                    if(null != bidRequestImpressionDTO.getBidfloor())
                    {
                        averageFloorPrice += bidRequestImpressionDTO.getBidfloor().floatValue();
                                                                                    ...
                                                                                    throws URLFieldProcessingException
    {
        ...
        BidRequestDeviceDTO bidRequestDeviceDTO = bidRequestParentNodeDTO.getDevice();
        ...
        String dpidMd5 = bidRequestDeviceDTO.getDpidmd5();
        String dpidSha1 = bidRequestDeviceDTO.getDpidsha1();
        String macSha1 = bidRequestDeviceDTO.getMacsha1();
        String macMd5 = bidRequestDeviceDTO.getMacmd5();
                                            ...
                                            )
    {
        ...
        if(
           null != bidRequestPMPDTO.getDeals() &&
           bidRequestPMPDTO.getDeals().length > 0
          )
        {
            logger.debug("Deals are present in bid request and size is {} inside " +
                         "CreativeAndFloorMatchingRTBExchangeTwoDotThree",
                          bidRequestPMPDTO.getDeals().length);

            for(BidRequestDealDTO deal : bidRequestPMPDTO.getDeals())
            {
                Double floor = null;
