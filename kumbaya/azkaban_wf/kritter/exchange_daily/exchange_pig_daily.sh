...
input_file=${3}-*/exchange_hourly/part*

echo "##########################################################################################################"
echo "${2}/bin/pig -x local -f src/main/pig/exchange_rollup.pig  --param INPUT_FILES='${input_file}'  --param OUTPUT='${4}' --param rolluptype='${5}' --param PROCESS_TIME='${6}'"

exec ${2}/bin/pig -x local -f src/main/pig/exchange_rollup.pig  --param INPUT_FILES=${input_file} --param OUTPUT=${4} --param rolluptype=${5}  --param PROCESS_TIME="${6}" 
