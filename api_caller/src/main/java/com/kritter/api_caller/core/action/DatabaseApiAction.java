...
public class DatabaseApiAction implements ApiDatabaseAction
{
    private Logger logger;
    private DatabaseConnector databaseConnector;

    public DatabaseApiAction(Logger logger,DatabaseConnector databaseConnector)
    {
        this.logger = logger;
    ...
    public ApiResponseEntity fetchApiResponseFromDatabase(ApiRequestEntity apiRequestEntity)
    {
        ...
        DatabaseRequestEntity databaseRequestEntity = (DatabaseRequestEntity)apiRequestEntity;
        ...
        try
        {
            ...
            if(((DatabaseRequestEntity) apiRequestEntity).getIsQueryForModification())
            {
            ...
            else
            {
                ...
                while (resultSet.next()){
                    ...
                    for(String columnName :  databaseRequestEntity.getColumnNamesToRead()){
                        ...
                        logger.debug("The database action is for retrieval, column read: " + columnName
                                     + "value read: " + value);
                    }
        ...
        {
            logger.error("SQLException inside DatabaseApiAction execution method,result could not be" +
                         " retrieved ", sqle);
