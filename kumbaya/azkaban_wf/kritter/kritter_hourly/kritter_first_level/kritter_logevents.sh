...
echo "##########################################################################################################"
echo "${2}/bin/pig -x local -f src/main/pig/logevents.pig --param OUTPUT='${4}' --param thrift_jar_path='${5}' --param PROCESS_TIME='${6}' --param POST_IMP_INPUT_FILES=${3}"

exec ${2}/bin/pig -x local -f src/main/pig/logevents.pig  --param OUTPUT=${4} --param thrift_jar_path=${5} --param PROCESS_TIME="${6}" --param POST_IMP_INPUT_FILES=${3}
