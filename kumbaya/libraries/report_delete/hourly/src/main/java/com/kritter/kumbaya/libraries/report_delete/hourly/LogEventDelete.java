...
public class LogEventDelete implements IReportDelete {
    private static Logger m_logger = Logger.getLogger(LogEventDelete.class);
    ...
    public void delete(String dbhost, String dbuser, String dbpwd,
        ...
        try{
            ...
            if((null != impression_time) && (!"".equals(impression_time))){
                System.out.println("delete from "+tablename+" where time <= '"+impression_time+"'");
                stmt.execute("delete from "+tablename+" where time <= '"+impression_time+"'");
            }
            conn.commit();
    ...
    public static void main(String args[]) throws Exception{
         ...
         */
        if(args.length == 8){
            LogEventDelete hd  = new LogEventDelete();
