...
public class TrackingEventCrud {
    private static final Logger LOG = LoggerFactory.getLogger(TrackingEventCrud.class);

    public static JsonNode get_data(Connection con,TrackingEvent trackingEvent, 
            boolean exportAsCSV, String absoluteFileName){
        if(con == null || trackingEvent == null){
            return null;
        }
        ...
        FileWriter fw =  null;
        try{
            TrackingEventHelper leh = new TrackingEventHelper();
            String queryString = leh.createQueryString(trackingEvent);
            if(queryString == null){
                return null;
            }
            ...
            if(exportAsCSV){
                ...
                bw.write(leh.creatHeadereCsv(trackingEvent.getCsvDelimiter()));
                bw.write("\n");
            }else{
                ...
                if(exportAsCSV){
                    ...
                    for(String key : leh.projectionMap.keySet()){
                        if(isFirst){
                            ...
                            bw.write(trackingEvent.getCsvDelimiter());
                        }
