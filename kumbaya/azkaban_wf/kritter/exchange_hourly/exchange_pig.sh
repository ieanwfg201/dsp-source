...
MAPRED=/var/data/kritter/pig_tmp/exchange_`date +%s`
pig_script=exchange
...
echo "##########################################################################################################"
echo "${2}/bin/pig -x local -f src/main/pig/exchange.pig  --param INPUT_FILES='${3}' --param OUTPUT='${4}' --param thrift_jar_path='${5}' --param PROCESS_TIME='${6}' --param tz='${8}'"

exec ${2}/bin/pig -x local -f src/main/pig/exchange.pig  --param INPUT_FILES=${3} --param OUTPUT=${4} --param thrift_jar_path=${5} --param PROCESS_TIME="${6}" --param tz=${8}
