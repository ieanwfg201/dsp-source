...
public class UserDelete implements IReportDelete {
    private static Logger m_logger = Logger.getLogger(UserDelete.class);
    ...
    public void delete(String dbhost, String dbuser, String dbpwd,
        ...
        try{
            ...
            if((null != impression_time) && (!"".equals(impression_time))){
                System.out.println("delete from "+tablename+" where processing_time <= '"+impression_time+"'");
                stmt.execute("delete from "+tablename+" where processing_time <= '"+impression_time+"'");
            }
            conn.commit();
