...
public class DirectPublisherClick implements IPrepareTransaction {

    private static final Logger LOG = LoggerFactory.getLogger(DirectPublisherClick.class);
    ...
    public void prepare_campaign_budget(List<Billing> billingLogList, Connection con,
        ...
        try{
            ...
            if(marketPlaceRset.next()){
                ...
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_MP_NF, BillingType.CPC);
                return;
            }
            if(!(marketplace_id == MarketPlace.CPC.getCode())){
                /*Simply return as the Campaign is not CPC*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_SERVED_CAMP_NOT_CPC, BillingType.CPC);
                preventTransation = true;
                return;
            }
            ...
            if(internal_total_remaining_budget <= 0){
                ...
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_INTR_Tbudget_NoF, BillingType.CPC);
                preventTransation = true;
                return;
            }else if(pirr.getInternal_max_bid() > internal_total_remaining_budget){
                /*Max Bid > internal total remaining*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_MAX_BID_GT_INTR_Tbudget, BillingType.CPC);
                preventTransation = true;
                return;
            }
            if(adv_total_remaining_budget <= 0){
                /*Adv Total Remaining Budget NO FUND*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_ADV_Tbudget_NoF, BillingType.CPC);
                preventTransation = true;
                return;
            }else if(pirr.getAdvertiser_bid() > adv_total_remaining_budget ){
                /*Max Bid > advertiser total remaining*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_MAX_BID_GT_ADV_Tbudget, BillingType.CPC);
                preventTransation = true;
                return;
            }
            if(internal_daily_remaining_budget <=0) {
                /*Internal Daily Remaining Budget NO FUND*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_INTR_Dbudget_NoF, BillingType.CPC);
                preventTransation = true;
                return;
            }else if(pirr.getInternal_max_bid() > internal_daily_remaining_budget){
                /*Max Bid > internal daily remaining*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_MAX_BID_GT_INTR_Dbudget, BillingType.CPC);
                preventTransation = true;
                return;
            }
            if(adv_daily_remaining_budget <=0 ){
                /*Adv Daily Remaining Budget NO FUND*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_ADV_Dbudget_NoF, BillingType.CPC);
                preventTransation = true;
                return;
            }else if(pirr.getAdvertiser_bid() > adv_daily_remaining_budget ){
                /*Max Bid > adv daily remaining*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_MAX_BID_GT_ADV_Dbudget, BillingType.CPC);
                preventTransation = true;
                return;
            }
    ...
    public void prepare_account_budget(List<Billing> billingLogList, Connection con,
        ...
        try{
            ...
            if(accountResultSet.next()){
                ...
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_ACCOUNT_NF, BillingType.CPC);
                preventTransation = true;
                return;
            }
            if(internal_balance <= 0){
                /*Account NoF*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_ACCOUNT_NoF, BillingType.CPC);
                preventTransation = true;
                return;
            }
            if(pirr.getAdvertiser_bid() > adv_balance){
                /*Advertiser Bid Greater than adv bal no fund*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_ADV_BID_GT_ADV_BAL, BillingType.CPC);
                preventTransation = true;
                return;
            }
            if(pirr.getInternal_max_bid() > internal_balance){
                /*Internal Bid Greater than adv bal no fund*/
                AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_ADV_BID_GT_INT_BAL, BillingType.CPC);
                preventTransation = true;
                return;
            }
        }catch(Exception e){
        ...
        try{
            ...
            if(payoutRset.next()){
                ...
                if(siteRset.next()){
                    ...
                    /*Site Id Not Found*/
                    AddBilling.createNaddTerminatedBillingObject(billingLogList, pirr, BillingTerminationReason.DPCLK_SITE_ID_NF, BillingType.CPC);
                    preventTransation = true;
                    return;
                }
            }
