...
public class AdStatsBaseEntity {
    ...
    protected AdStats adstatsEntity = new AdStats();

    public AdStatsBaseEntity(AdStats adstatsEntity) {
        this.adstatsEntity = adstatsEntity;
        if (adstatsEntity == null
                || adstatsEntity.getStart_time_str() == null
                || "".equals(adstatsEntity.getStart_time_str())) {
            Date date = DateUtils.addDays(new Date(), -1);
            start_date = TimeManipulator.convertDate(date,
                    "yyyy-MM-dd 00:00:00", timezoneid);
        } else {
            this.start_date = adstatsEntity.getStart_time_str();
        }
        if (adstatsEntity == null
                || adstatsEntity.getEnd_time_str() == null
                || "".equals(adstatsEntity.getEnd_time_str())) {
            Date date = DateUtils.addDays(new Date(), -1);
            end_date = TimeManipulator.convertDate(date, "yyyy-MM-dd 00:00:00",
                    timezoneid);
        } else {
            this.end_date = adstatsEntity.getEnd_time_str();
        }

    }

    public AdStatsBaseEntity() {
        this.adstatsEntity = new AdStats();
        Date date = DateUtils.addDays(new Date(), -1);
    ...
    public void setStart_date(String start_date) {
        adstatsEntity.setStart_time_str(start_date);
    }

    public String getStart_date() {
        String str = adstatsEntity.getStart_time_str();
        if (str == null || "".equals(str)) {
    ...
    public void setEnd_date(String end_date) {
        adstatsEntity.setEnd_time_str(end_date);
    }

    public String getEnd_date() {
        String str = adstatsEntity.getEnd_time_str();
        if (str == null || "".equals(str)) {
            return TimeManipulator.convertDate(DateUtils.addDays(new Date(), -1),
                    "yyyy-MM-dd 00:00:00", timezoneid);
        }
