...
class KrritterWinLogAdapter(GenericInputLogAdapter):
    def __init__(self, dimIdNameMap, winBidFieldId, timeDimension = -1, timePerWindow = 0, dimDelimiter = ',', fieldsDelimiter = u'\u0001'):
        """ Constructor

        :param dimIdNameMap: Map containing the ids->name mapping of the dimensions to be extracted
        :type dimIdNameMap: map {int->str}
        :param winBidFieldId: Position of win bid field
        :type winBidFieldId: int
        :param timeDimension: The dimension id of time (if present). Special handling is required for time
        :type timeDimension: int
        :param timePerWindow: If time is one of the dimensions, the length (in minutes) of each time window
        :type timePerWindow: int
        :param dimDelimiter: delimiter for the dimension
        :type dimDelimiter: str
        :param fieldsDelimiter: delimiter between dimensions and fields and among fields too
        :type fieldsDelimiter: str
        """
        self.dimIdNameMap = dimIdNameMap
        self.winBidFieldId = winBidFieldId
    ...
    def getFields(self, requestResponse, dimDelimiter, fieldsDelimiter) :
        """
        Gets the dimensions and fields in a string which looks as follows :
        <dim1><dimDelimiter><dim2><dimDelimiter><dim2><dimDelimiter>..<fieldsDelimiter><field1><fieldsDelimiter><field2>...
        """
        ...
        #TODO : Get the bid from the request response. Should not be 1
        fieldList += 1
        return fieldList
