...
proj_data = FOREACH decoded_data GENERATE AdservingRequestResponse.terminationReason as terminationReason, 
    ...
    AdservingRequestResponse.pub_inc_id as pub_inc_id;

group_data = GROUP proj_data BY (time,pub_inc_id,terminationReason);

flatten_data = FOREACH group_data GENERATE FLATTEN(group), COUNT(proj_data);

STORE flatten_data INTO '$OUTPUT/reqterminationreason'  USING PigStorage('');
