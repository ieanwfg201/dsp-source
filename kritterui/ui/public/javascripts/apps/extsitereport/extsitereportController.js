...
controller('extsitereportController',function ($scope, $http, ngTableParams, $modal) {
	$scope.initComplete = false;
	$scope.getCSV = false;
	
	$scope.init = function(){
		$scope.initComplete = true;
			$('#total_request').prop("checked",true);
			$('#total_impression').prop("checked",true);
			$('#total_csc').prop("checked",true);
			$('#billedcsc').prop("checked",true);
			$('#total_click').prop("checked",true);
			$('#billedclicks').prop("checked",true);
			$('#total_win').prop("checked",true);
			$('#conversion').prop("checked",true);
			$('#supplyCost').prop("checked",true);
			$('#exchangepayout').prop("checked",true);
			$('#networkpayout').prop("checked",true);
			$('#demandCharges').prop("checked",true);
			$('#exchangerevenue').prop("checked",true);
			$('#networkrevenue').prop("checked",true);
			$('#cpa_goal').prop("checked",true);
			$('#total_bidValue').prop("checked",true);
			$('#total_win_bidValue').prop("checked",true);
	}
	...
	var tbparams = new ngTableParams({
      ...
      getData: function($defer, params) {   
		...
		$('#preloader').removeClass("hidden"); 
      	var action_url = $('#extsitereportConfigForm').attr('action'); 
      	if($scope.getCSV){
       		action_url = action_url + "/CSV";
       	}
   		var data = $('#extsitereportConfigForm').serialize();
	...
	// exchanges and ext sites
	$scope.exchanges_msmodel = Object.create(MultiSelectModel);
	$scope.exchanges_msmodel.init($scope.exchange_url, 'valueArray', true, true);
	...
	$scope.exchanges_msmodel.addDependent("extsites", $scope.extsites_msmodel);
