...
controller('trackingeventController',function ($scope, $http, ngTableParams, $modal) {
	...
	$scope.checkuncheckmetrics = function(checkvalue){
		$('#total_event').prop("checked",checkvalue);
	}
	...
	$scope.teventtype_url = "/metadata/options/teventtype";
	...
	$scope.country_msmodel.addDependent("carrier", $scope.carrier_msmodel);
	...
	$scope.teventtype_msmodel = Object.create(MultiSelectModel);
	$scope.teventtype_msmodel.init($scope.teventtype_url, 'valueArray', false, true);
