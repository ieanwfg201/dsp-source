...
STORE flatten_data_store INTO '$OUTPUT/daily_first_level' USING PigStorage('');
...
limited_flatten_data_store = FOREACH limited_flatten_data GENERATE group::PROCESS_TIME, group::time, group::pub_inc_id, group::siteId, group::deviceManufacturerId, group::deviceOsId, group::countryId, group::countryCarrierId, group::adId, group::campaignId, group::adv_inc_id, total_request, total_impression, total_bidValue, total_click, total_win, total_win_bidValue, total_csc, demandCharges, supplyCost, earning, conversion, bidprice_to_exchange, cpa_goal, exchangepayout, exchangerevenue, networkpayout, networkrevenue, billedclicks,billedcsc;

STORE limited_flatten_data_store INTO '$OUTPUT/limited_daily_first_level' USING PigStorage('');


ext_site_id_inp = LOAD '$EXT_INP_FILES' USING PigStorage('') AS (siteId:int, supply_source_type:int, ext_supply_url:chararray, ext_supply_id:chararray, ext_supply_name:chararray, ext_supply_domain:chararray, ext_supply_attr_internal_id:int,deviceOsId:int);

ext_site_id_grp = GROUP ext_site_id_inp BY (siteId, supply_source_type, ext_supply_url, ext_supply_id, ext_supply_name, ext_supply_domain, ext_supply_attr_internal_id, deviceOsId);

ext_site_id_gen = FOREACH ext_site_id_grp GENERATE FLATTEN(group), COUNT(ext_site_id_inp) as cnt;

ext_site_id_group_gen  = FOREACH ext_site_id_gen GENERATE group::siteId, group::supply_source_type, group::ext_supply_url, group::ext_supply_id, group::ext_supply_name, group::ext_supply_domain, group::ext_supply_attr_internal_id, cnt as cnt, group::deviceOsId;

ext_site_id_filter = FILTER ext_site_id_group_gen BY cnt >=100;

STORE ext_site_id_filter INTO '$OUTPUT/daily_externalsite.gz' USING PigStorage('');
